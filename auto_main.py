...
class Application(tk.PanedWindow):
    ...
    def open_file(self, event=None):
        ...
        [self.line_vals[i].set(n) for i, n in enumerate([365, 422, 470])]
        [self.update_scales(None, i) for i in range(N)]
        [self.channel_listbox.selection_clear(i) for i in [1, 2, 3]]

    def auto_analysis(self, event=None):
        if self.img != None:
            top = int(331*self.canvas.aspect) ## changed from 390
            bottom = int(502*self.canvas.aspect) ## chamged from 560
            left = int(137*self.canvas.aspect) ## changed from 32
            right = int(230*self.canvas.aspect) ## changed from 120
            last = int(self.img.size[0]*self.canvas.aspect)
            spacing = int(355*self.canvas.aspect) ## changed from 101
...
if __name__ == '__main__':
    ...
    root.state('normal') ## changed from root.state('zoomed')
